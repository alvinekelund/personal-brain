...
**Tree integrity** — the person-rooted hierarchy is checked on every `brain doctor`: orphans, nodes with more than one `part_of` parent, categories not hanging off the person or a parent category, cycles, edges left behind by a deleted node, legacy task nodes, near-duplicate names (including a bare first name vs a full name, and two same-type nodes that `brain index` maps to one vault file), near-duplicate meanings (two same-type nodes whose stored embeddings agree at 0.89+ cosine — `Current Semester Start` beside `SM Data Science Start` — no model call, parent/child pairs excluded), nodes without embeddings, and categories with more direct children than the sub-grouping threshold (the cure is named: `brain subgroup`), a fact used as a parent (a fact is an attribute, a leaf by design — `brain move` its children to the entity the fact is about), or the same fan-out under an event, organization or project (a flat list wearing one node per name — `brain merge` / `brain forget`; the extractor is told to keep a list of sponsors, vendors or attendees as one fact), and top-level areas with two or fewer descendants once the graph holds twenty nodes (`Family` beside `Relationships`, an empty `Health`: `brain move` it under the broader area or `brain merge` it away). `brain repair` fixes the structural ones deterministically and never deletes a node. Nothing ingests before `brain setup` names the owner — `brain add`, the MCP tool, the web page and the capture hook all refuse (without a root every node would be an orphan). Content gets its own check: a node still saying "plans to" or "is currently" thirty days after it was written is a claim nobody re-read (a mail backfill wrote "Alvin plans to relocate to Boston" three months late); `brain stale` lists them and `brain describe` restates what is true now (a plan dated "as of …" is deliberate and never listed); a future claim that names its date ("will begin classes on September 2") is stale the day after, whatever its age. Ingest and reorganize keep the invariant at write time: a planned parent replaces the old one; `brain merge` keeps one parent (the other becomes a cross-link) and `brain move` re-homes a node under the same rules.
...
# Backup / portability
brain backup [--label X] [--keep N]   # consistent brain.db snapshot into ~/.personal-brain/backups/ (SQLite backup API), pruned to N
