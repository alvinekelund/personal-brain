...
**Importance-weighted forgetting** — each node has an `importance` (0–1, scored at ingest) and a type half-life. An event whose content names a date still ahead does not fade before that date (a hackathon three weeks away was at weight 0.89). A forgotten node comes back the moment new knowledge names it — as a parent or as itself — instead of leaving the new node outside the active tree. Important nodes decay much slower and never auto-archive; one-off details still fade. People and organisations never decay — but only while they matter: below importance 0.4 a one-off sponsor or a passing acquaintance is remembered like a concept, not forever. Accessing a node resets it to 1.0 and propagates a freshness boost up its branch. Decay runs automatically on every CLI call but is purely time-based: nodes and edges carry a `last_decayed` clock and only the interval since then is applied, so call frequency never compounds it. Hierarchy (`part_of`) edges may fade but are never pruned — the spine survives long silences.
...
**Action layer** — `LOOPS.md` (one loop per line in a strict grammar: id, title, due, owner alvin/claude/waiting:<who>, area, prio, next action — one concrete step; lint warns once when next actions run past 200 characters) and `DECISIONS.md` (append-only: decision, why, rejected, revisit-if). Both are written only by `brain loop` / `brain decide`; `lint` fails on hand edits and a git pre-commit hook rejects removals from the decision ledger. NOW.md's "hot" section is rendered from the loops. `brain today` turns them into a deterministic action card (countdowns, aging waits, Claude-owned loops, top 3 next actions) and `brain doctor` checks the whole wiring — binary, DB, key, vault freshness, the last ambient capture, the last recorded phone brief (`brain today --brief` logs every line it produces), hooks, MCP registration, scheduled tasks — so a broken brain announces itself at session start instead of failing silently.

**Tree integrity** — the person-rooted hierarchy is checked on every `brain doctor`, and every finding names its cure:

- *Structure* (`brain repair` fixes these deterministically and never deletes a node): orphans, nodes with more than one `part_of` parent, categories not hanging off the person or a parent category, cycles, edges left behind by a deleted node, cross-links touching a category (structure, not knowledge — a merge used to leave `relates_to Education` behind), passive relations written backwards (`Alvin studied_by Harvard` — the agent belongs at the target end; `add_edge` turns new ones round), a `relates_to` doubling a `part_of` between the same two nodes (`add_edge` reinforces the spine instead), legacy task nodes, nodes without embeddings (`brain reindex`).
- *One thing, one node*: near-duplicate names (a bare first name vs a full name; two same-type nodes that `brain index` maps to one entity file), near-duplicate meanings (two same-type nodes whose stored embeddings agree at 0.89+ cosine — `Current Semester Start` beside `SM Data Science Start` — no model call, parent/child pairs excluded) → `brain merge`.
- *Shape*: a category with more direct children than the sub-grouping threshold (`brain subgroup`); the same fan-out under an event, organization or project — a flat list wearing one node per name (`brain merge` / `brain forget`; the extractor keeps a list of sponsors, vendors or attendees as one fact); a fact used as a parent — a fact is an attribute, a leaf by design (`brain move` its children to the entity); top-level areas with two or fewer descendants once the graph holds twenty nodes — `Family` beside `Relationships`, an empty `Health` (`brain move` it under the broader area or `brain merge` it away).
- *Content* (`brain stale`, then `brain describe`): a node still saying "plans to" or "is currently" thirty days after it was written is a claim nobody re-read (a mail backfill wrote "Alvin plans to relocate to Boston" three months late; a plan dated "as of …" is deliberate and never listed); a future claim that names its date ("will begin classes on September 2") is stale the day after, whatever its age; so is content that leans on a loop that has since closed ("Venmo is holding the money (L-063)").
- *Coverage* (D-014): an important node with no vault file behind it is knowledge living only in the graph — add an alias to the file that covers it, or write the file.

Nothing ingests before `brain setup` names the owner — `brain add`, the MCP tool, the web page and the capture hook all refuse (without a root every node would be an orphan). Ingest and reorganize keep the invariant at write time: a planned parent replaces the old one; `brain merge` keeps one parent (the other becomes a cross-link) and `brain move` re-homes a node under the same rules.
...
**The phone channel is an iMessage to yourself, with an optional email copy** — `brain push` hands a line to this Mac's Messages.app, which sends it to your own iMessage handle (your Apple ID email, set once with `--set-to`; iMessage only, never the SMS relay, so no carrier charges), and, when `--set-email` names a second address, Mail.app sends the same line there from the account you chose with `--from`; `--brief` sends the same line as `brain today --brief`. The morning brief and the scheduled tasks' urgent notices use it, every attempt lands in `~/.personal-brain/push.log`, and `brain doctor` warns when a day passes without a push or the last one failed. The Claude app's push notification stays as the fallback: it reaches the phone only while Remote Control is connected and leaves no trace.
...
brain push [TEXT] [--brief] [--set-to H] [--set-email A --from F] [--no-email]   # iMessage to your Apple ID handle via Messages.app, never SMS, plus an optional email copy via Mail.app; logged in push.log
...
| Nodes | What happens |
|---|---|
| **up to ~10k** | Everything as is. SQLite + WAL, packed float32 embeddings (12 KB per 3072-dim vector; JSON text was 42 KB) scanned in Python, vis-network for the graph, one Gemini call per stage. |
