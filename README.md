...
- *Structure* (`brain repair` fixes these deterministically and never deletes a node): orphans, nodes with more than one `part_of` parent, categories not hanging off the person or a parent category, cycles, edges left behind by a deleted node, cross-links touching a category (structure, not knowledge — a merge used to leave `relates_to Education` behind), legacy task nodes, nodes without embeddings (`brain reindex`).
